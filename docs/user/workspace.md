...
workspace全体はchecksummed ZIPとしてexportできます。Markdown/YAML、設定、append-only eventを含み、
再生成可能なcache、lock、`.work-smarter/secrets/`は含みません。
...
import先は空でなければなりません。importは展開前にmanifest、各fileのSHA-256、archive pathを検査し、
改ざん・欠落・path traversalを拒否します。`migrate` はworkspace lockを取得し、全documentを現在の
public schemaで検証・再保存します。migration前には必ずexportしてください。
