...
`ws project`（互換alias: `ws pm`）はGTDの複数アクション・プロジェクトとは別の、管理対象プロジェクト用機能です。phase、work package、milestone、依存関係、クリティカルパス、QCD、risk/issue/decision、要求とV&V証跡のstructured stateをDatabase、narrativeをMarkdownで管理します。
